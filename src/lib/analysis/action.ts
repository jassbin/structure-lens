...
## 你的任务（顺序很重要）
1. **【最重要，先写 answer】先给用户一个有实质内容的"直接回复"，而不是只表态。**
   - 若是**追问型**（问"为什么/凭什么/能不能再具体"）：在 answer 里**正面、具体地回答那个问题本身**——往更深一层给出机制/依据/更具体的做法，真正帮到"作为${perspective.label}的用户"。严禁复述原结论，严禁只说"你这个问题问得好"。
   - 若是**反驳型**：在 answer 里给出**完整、经得起追问的推理**——采纳就讲清用户哪点成立、为何改；维持就讲清用户的反驳为何不成立、原建议凭什么依然站得住（要给推理链，不是甩一句"你的表述不足以推翻"）；部分采纳就讲清接受哪块、保留哪块各自的依据。
   - **answer 里绝不允许出现"你部分成立但…""已考虑你的意见"这类只表态不解释的空话。** answer 必须是用户能读、有信息量、针对他这句话本身的回应。
2. 再据此**表态** stance：absorb（采纳）/ compromise（部分采纳）/ hold（维持），并在 reason 里做结构化归档（简短即可，实质内容在 answer 里）。
   - 判定标准只有一个：用户的反驳/追问有没有客观依据、站不站得住。别为了让用户舒服而附和，也别为了显得中立而和稀泥。追问型通常应判 absorb/compromise（它往往指出了原方案"没讲透/该更具体"），只有基于误解才 hold（且 hold 也要在 answer 里把"为什么"讲清）。
3. 若 stance 是 absorb 或 compromise（别偷懒）：
   - 先重写「${label}」这一节，把 answer 里的新认识落进去；
   - 然后**逐一重新审视每个下游步（${downstreamLabels.join("、") || "无下游步"}）**：只要用户这个新观点会影响到它（痛点变了→分诊要变→最小行动要变），就**必须把那个下游步一并重写**并回传完整新内容；确实不受影响的下游步可不回传（保持原样）。
   - **绝不允许**只改「${label}」却放任明明受影响的下游步不管——那样方案会自相矛盾。
   - 若 hold：steps 留空对象 {}（但 answer 仍要把道理讲透）。

## 输出格式（只输出一个 JSON，不要 markdown）
{
  "answer": "【最重要，先写这个】给用户的直接回复：追问就正面回答那个'为什么'、给更深机制或更具体做法；反驳就给出为何采纳/为何维持的完整推理。必须有信息量，不能是只表态的空话。",
  "stance": "absorb|compromise|hold",
  "reason": "表态的简短归档理由",
